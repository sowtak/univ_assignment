...
class YOLP
  ...
  def coordinate(address)
    coord = nil
    open("https://map.yahooapis.jp/geocode/V1/geoCoder?query=#{URI.escape(address)}&appid=#{@app_id}&output=json"){|f|
  ...
  def address(coord)
    ...
    open("https://map.yahooapis.jp/geoapi/V1/reverseGeoCoder?lon=#{cArray[0]}&lat=#{cArray[1]}&appid=#{@app_id}&output=json"){|f|
      json = JSON.parse(f.read)
      if json['ResultInfo']['Count'] > 0
